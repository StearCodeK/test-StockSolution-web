  ...
  async _validateAndCleanData(df, requiredColumns, operationName) {
    ...
    for (const row of df) {
      ...
      if (typeof dateStr === "number") {
        // Serial de Excel
        // Excel serial date: días desde 1899-12-30 (ajustado)
    ...
    if (invalidRows.length > 0) {
      logger.warn(
        `⚠ Advertencia: ${invalidRows.length.toLocaleString()} fechas inválidas en ${operationName}`
      );
      await this._saveFailedBatch(
        invalidRows,
        `invalid_dates_${operationName}`
      );
    }

    logger.info(
      `📦 Total de registros válidos en ${operationName}: ${validRows.length.toLocaleString()}`
    );
    return validRows;
  }
